...
Muestra de validación manual → genera data/label_validation_sample.csv con muestreo proporcional al corpus real. Si el archivo ya existe con anotaciones manuales, no lo sobreescribe.
...
def save_validation_sample(df: pd.DataFrame, n: int, seed: int) -> None:
    """
    Guarda una muestra estratificada PROPORCIONAL en CSV para anotación manual.
    La distribución refleja la del corpus real (no balanceada por clase).
    La muestra contiene el texto original, el texto limpio y las etiquetas
    automáticas, más una columna vacía 'label_manual' para anotar manualmente.

    Si el archivo ya existe con anotaciones manuales, no lo sobreescribe.
    El investigador anota la columna 'label_manual' y calcula el acuerdo
    con compute_kappa().

    Kappa de referencia (validación manual del TFM):
      κ humano vs pysentimiento:  0.4291 (moderado)
      κ humano vs cardiffnlp:     0.3690 (regular)
      κ inter-sistema en muestra: 0.7214 (sustancial)
    """
    import os   # Para verificar si el archivo ya existe

    output_path = "data/label_validation_sample.csv"

    # Si ya existe el archivo con anotaciones manuales, no sobreescribir
    # Esto protege el trabajo de anotación humana ya realizado
    if os.path.exists(output_path):
        try:
            existing = pd.read_csv(output_path)
            if "label_manual" in existing.columns and existing["label_manual"].notna().any():
                if (existing["label_manual"].astype(str).str.strip() != "").any():
                    print(f"ℹ️  Muestra de validación ya existe con anotaciones manuales: {output_path}")
                    print("   → No se sobreescribe. Elimina el archivo manualmente si quieres regenerarlo.")
                    return
        except Exception:
            pass   # Si hay error al leer, continuamos y regeneramos el archivo

    # Seleccionar columnas disponibles para la muestra
    cols = ["clean_text", "label_sentiment", "label_confidence"]
    if "full_text" in df.columns:
        cols.insert(0, "full_text")          # Texto original para facilitar la anotación humana
    if "label_cardiffnlp" in df.columns:
        cols.append("label_cardiffnlp")      # Segundo anotador para referencia

    df_cols = df[cols].copy()

    # Muestra estratificada PROPORCIONAL a la distribución real del corpus
    # Esto es metodológicamente más correcto que el muestreo balanceado (n//3 por clase)
    # porque refleja la distribución real del problema de clasificación
    distribucion = df_cols["label_sentiment"].value_counts(normalize=True)
    partes = []
    for clase, proporcion in distribucion.items():
        k = max(1, round(n * proporcion))    # Tweets proporcionales a cada clase
        subset = df_cols[df_cols["label_sentiment"] == clase]
        k = min(len(subset), k)              # No pedir más de los disponibles
        if k > 0:
            partes.append(subset.sample(k, random_state=seed))

    sample = pd.concat(partes, ignore_index=True)
    ...
    try:
        ...
        print("  Cierra el archivo si está abierto en Excel u otro programa y vuelve a correr.\n")
        return

    print(f"✅ Muestra de validación guardada: {output_path} ({len(sample)} tweets)")
    print(f"   Distribución proporcional: {dict(sample['label_sentiment'].value_counts())}")
